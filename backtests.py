...
def backtest_strategy1(tradingApp:TradingApp,strategy_vars:dict):
        ...
        excel = Excels(other_vars["excle_path_backtest"],False,contracts)
        ...
        for ticker in dataFramesDict.keys():
                ...
                dataFramesDict[ticker] = indicators.Zscore(dataFramesDict[ticker],strategy_parameters["ZscoreWindow"],strategy_parameters["smaZwindow"])
                ...
                dataFramesDict[ticker].dropna(inplace=True)#important for backtest only 
                result[ticker] = {}
        ...
        for ticker in dataFramesDict.keys():
                ...
                for i in range(1,len(current_dataFrame)):
                        ...
                        current_smaZ= current_dataFrame["smaZ"].iloc[i]
                        current_price=current_dataFrame["close"].iloc[i]
                        current_date=current_dataFrame.index[i]
                        current_signal=strategy_func(current_Zscore,current_smaZ)
                        ...
                        if current_position == "":
                                if current_signal == "BUY":
                                        ...
                                        print(f"OPEN TRADE : {current_position} || PRICE : {current_price} || DATE : {current_date} || STRATEGY SIGNAL : {current_signal} || Zscore : {current_Zscore} || smaZ : {current_smaZ}")
                                        excel.write_line_strategy1(current_date,current_price,"BUY",current_signal,current_Zscore,current_smaZ,sum(current_return),Utilities.calculate_compound_returns(current_return))
                                elif current_signal == "SELL" :
                                        current_position="SELL"
                                        current_tradeCount+=1
                                        current_start_price=current_price
                                        print(f"OPEN TRADE : {current_position} || PRICE : {current_price} || DATE : {current_date} || STRATEGY SIGNAL : {current_signal} || Zscore : {current_Zscore} || smaZ : {current_smaZ}")
                                        excel.write_line_strategy1(current_date,current_price,"SELL",current_signal,current_Zscore,current_smaZ,sum(current_return),Utilities.calculate_compound_returns(current_return))
                        else :
                                if current_position == "BUY":
                                        if current_signal == "SELL":
                                                ...
                                                print(f"CLOSE TRADE || PRICE : {current_price} || DATE : {current_date} || STRATEGY SIGNAL : {current_signal} || Zscore : {current_Zscore} || smaZ : {current_smaZ}")
                                                excel.write_line_strategy1(current_date,current_price,"SELL",current_signal,current_Zscore,current_smaZ,sum(current_return),Utilities.calculate_compound_returns(current_return))
                                                # GO SHORT 
                                                current_position="SELL"
                                                current_tradeCount+=1
