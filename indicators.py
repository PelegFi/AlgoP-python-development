...
class Indicators():
    ...
    def RSI(dataFrame: pd.DataFrame, window: int):
        df = dataFrame.copy()
        ...
        for i in range(len(df)):
            if i < window:
                ...
                avg_gain.append(((window-1)*avg_gain[i-1] + gain[i])/n)
                avg_loss.append(((window-1)*avg_loss[i-1] + loss[i])/n)
    ...
    def Zscore(dataFrame:pd.DataFrame,ZscoreWindow,MAwindow) :
        ...
        df["smaZ"]=df["Zscore"].rolling(MAwindow).ewm(span=MAwindow,min_periods=MAwindow).mean()
